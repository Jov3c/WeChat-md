...
顶部只保留统一的“风格”入口。每套风格同时决定配色、字体、字号、行距，以及标题、章节、引用和列表的排版结构。切换风格**不会替换正文，也不会新建文章**。

1. 打开顶部“风格”菜单
2. 选择教程、新闻、资讯、分享、产品或长文等风格
3. 在预览区检查配色与章节节奏
4. 不满意时继续切换，正文始终保留

### 新建文章时选择模板

点击“新建文章”后，可以从空白文章开始，也可以选择教程、新闻、资讯、经验分享或产品介绍模板。模板提供可以直接填写的正文骨架，并只在新建文章时使用，不会覆盖当前原稿。
...
如果当前正文结构很适合某类文章，可以保存为自定义模板；固定结尾、作者介绍、关注提示等可重复片段，则适合保存为组件。组件插入正文后仍然是普通 Markdown，可以继续编辑。

| 功能 | 解决的问题 |
| --- | --- |
| 模板 | 用预先准备的正文骨架新建文章 |
| 风格 | 统一配色、字体、排版结构与阅读节奏 |
| 组件 | 复用固定结尾、提示框和常用内容块 |
| 图片资源 | 集中管理上传和提取到的图片 |
| 历史版本 | 找回重要节点，降低误改风险 |
...
现在试着改动这篇文章、切换一次风格，再用模板新建一篇文章。你会发现：从写作、排版到发布，所有步骤都可以留在同一个工作区里完成。`
